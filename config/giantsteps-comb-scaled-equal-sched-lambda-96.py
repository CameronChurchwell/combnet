...
PARAM_GROUPS = {
    ...
    'main': {'lr': 1e-4*main_start_factor},
    'f0': {'lr': 1e-4*f0_start_factor}
}
...
def main_lr(i):
    return max(0.95 ** i,1/main_start_factor)
def f0_lr(i):
    return max(0.95 ** i, 1/f0_start_factor)
SCHEDULER_FACTORY = torch.optim.lr_scheduler.LambdaLR
SCHEDULER_KWARGS = {'lr_lambda': [main_lr, f0_lr]}