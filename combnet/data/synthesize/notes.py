...
def generate_sequence(note_choices):


# ALL_NOTES = ["C", "F", "Bb", "Eb", "Ab", "Db", "Gb", "B", "E", "A", "D", "G"]
def notes(n=1000, note_choices=["C", "F", "Bb", "Eb"]):
    """
    Generate the notes synthetic dataset using pyfluidsynth
    """
    ...
    for i in tqdm.tqdm(range(0, n), desc='synthesizing notes dataset', total=n, dynamic_ncols=True):
        stem = str(i)
        # Generate plausible random MIDI
        # Select instrument
        # Synthesize audio
        # Create label
        # Save
        n_notes = randint(3, 10)
        notes = [choice(note_choices) for _ in range(0, n_notes)]
        bpm = randint(80, 160)
        instrument = choice(allowed_instruments)
        fl.program_select(0, soundfont, 0, instrument)
        notes_audio = torch.tensor(generate_notes(notes), dtype=torch.float32)[None]
        notes_audio /= abs(notes_audio).max() # normalize to [-1, 1]
        note_names = 
        torchaudio.save(dataset_dir / f'{stem}.wav', notes_audio, combnet.SAMPLE_RATE)
        with open(dataset_dir / f'{stem}.notes.json', 'w+') as f:
            ...
            json.dump(notes, f)
        with open(dataset_dir / f'{stem}.notes', 'w+') as f:
            f.write(label)
        with open(dataset_dir / f'{stem}.metadata', 'w+') as f:
            json.dump({
                ''
            })