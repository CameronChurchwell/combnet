...
@triton.jit
def _sinc_forward_kernel_fused(
    ...
    for block_window_offset in range(0, window_size, block_time):
        ...
        for ic_counter in range(0, in_channels, block_in_channels):
            ...
            for tap in range(1, n_taps+1, 1):
                # calculate sinc ratio of floor and ceil
                fractional_delays = tap * delays # 1 x block_out_channels x block_in_channels x 1
                floor_delays = tl.floor(fractional_delays+0.5).to(tl.int32) # 1 x block_out_channels x block_in_channels x 1
                ...
                for w in range(-10, 11, 1):
                    ...
                    tap_indices = indices - (floor_delays - w) # 1 x block_out_channels x block_in_channels x block_time
                    sinc_input = (k+w)*3.141592
                    # coefs = tl.sin(sinc_input)/sinc_input
                    coefs = tl.where(tl.abs(sinc_input)<0.001, 1.0, tl.sin(sinc_input)/sinc_input)
                    ...
                    x_tile = tl.load(x + x_indices, mask) * gains * coefs
...
@triton.jit
def _sinc_backward_kernel_fused(
    x, # batch x in_channels x time
    l, # out_channels x in_channels
    a, # out_channels x in_channels
    ...
    for ic_counter in range(0, in_channels, block_in_channels):
        ...
        for tap in range(1, n_taps+1, 1):
            ...
            for w in range(-10, 11, 1):
                ...
                tap_indices = indices - (floor_delays - w) # 1 x block_out_channels x block_in_channels x block_time
                sinc_input = (k+w)*3.141592
                approx = (tl.abs(sinc_input) < 0.000001)
                coefs = tl.where(approx, 0, (tl.cos(sinc_input)-tl.sin(sinc_input)/(sinc_input))/sinc_input)
                # apply channel and batch and channel offsets to indices
                x_indices = tap_indices + in_channel_indices * time \
                    + batch_indices * in_channels * time # block_batch x block_out_channels x block_in_channels x block_time
                # create mask based on indices components and bounds
                ...
                # x_tile = tl.load(x + x_indices, mask) * tap * gains * tl.where(w<=0, 1, -1)
                x_tile = tl.load(x + x_indices, mask) * tap * gains * coefs
                accumulator += x_tile
...
class _explicit_sinc_triton_fused(torch.autograd.Function):
    ...
    def forward(
        ...
        def grid(META):
            ...
            return grid_shape
        _sinc_forward_kernel_fused[grid](
    ...
    def backward(ctx, output_gradient: torch.Tensor):
        ...
        def grid(META):
            ...
            return grid_shape
        _sinc_backward_kernel_fused[grid](
            x=x, # batch x in_channels x time
            l=l, # out_channels x in_channels
            a=a, # out_channels x in_channels
...
def fractional_comb_fir_multitap_sinc_explicit_triton_fused(x, f0, a, sr, window_size=None, stride=None, n_taps=10):
    if x.dim() == 1: # time
        x = x[None, None]
    ...
    y = _explicit_sinc_triton_fused.apply(x, y, m, a, l, n_taps, window_size, stride)
    return y