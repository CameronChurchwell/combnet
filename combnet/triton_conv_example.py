...
class _conv:
    ...
    def _delta_x_ptr(
        ...
        delta_x = (
            window_unpack_h * stride_xh + window_unpack_w * stride_xw + r_inc * stride_xc
        )
        return delta_x
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 2, 1024, 1024, device=device, dtype=torch.float16)
    w = torch.randn(3, 2, 16, 16, device=device, dtype=torch.float16)
    ...
    y_target = torch.nn.functional.conv2d(
        x,
        w,
        stride=stride,
        groups=groups
